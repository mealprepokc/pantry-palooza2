  ...
  const markAsCooked = async (dish: RichSavedDish) => {
    ...
    try {
      ...
      const { error: insertErr } = await supabase.from('cooked_dishes').insert({
        ...
        calories_est: numeric(dish.calories_est),
        cost_est: knownCost,
      ...
      if (Platform.OS === 'android') {
        ...
        Alert.alert('Cooked', 'Dish moved to Cooked.');
      }
    } catch (error: any) {
      ...
      {savedDishes.length === 0 ? (
        ...
        <ScrollView style={styles.scrollView} contentContainerStyle={styles.content}>
          {savedDishes.map((dish) => {
            ...
            const toggle = () => setExpanded((prev) => ({ ...prev, [dish.id]: !prev[dish.id] }));
            const ingredients = parseIngredientList(dish.ingredients);
            const cost = numeric(dish.cost_est);
            const restaurant =
              numeric(dish.restaurant_cost_est) ?? (cost != null ? Math.round(Math.max(cost * 3.6, cost + 24) * 2) / 2 : null);
            const savings =
              numeric(dish.savings_est) ??
              (restaurant != null && cost != null ? Math.max(0, Math.round((restaurant - cost) * 100) / 100) : null);
            const calories = numeric(dish.calories_est);
            const instructions =
              typeof dish.instructions === 'string' && dish.instructions.trim().length > 0 ? dish.instructions.trim() : '';
              ...
              <View key={dish.id} style={styles.item}>
                <TouchableOpacity onPress={toggle} style={styles.itemHeader}>
                  ...
                  <View style={styles.itemActionCluster}>
                    ...
                    >
                      {markingCooked[dish.id] ? <ActivityIndicator size="small" color="#4ECDC4" /> : <ChefHat size={18} color="#4ECDC4" />}
                    </TouchableOpacity>
                ...
                {isOpen && (
                  <View style={styles.itemBody}>
                    ...
                    <View style={styles.itemStats}>
                      ...
                      {calories != null && <Text style={styles.statBadge}>~{Math.round(calories)} kcal</Text>}
                    </View>
